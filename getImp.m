...
if strcmp(mode, 'exact')
    ...
    n = size(x,2);
    % ensure that the gen second dimension makes sense
    ...
    Ni = zeros(k,n);
    for i = 1:k
        Ni(i,:) = sum((Lx - Lx(:, gen(1, (i-1)*n+1:i*n))).^2, 1) +1; 
    end
    ...
    for i = 1:n % for each training example
        for j = 1:k % for each target neighbour
            % get training examplex labelled differently
            idxs = find(y~=y(i));
